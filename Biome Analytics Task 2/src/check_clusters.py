...
check = merged[["Facility ID", "peer_group"]].merge(
    combined[["Facility ID", "Hospital Type", "Hospital Ownership", "Emergency Services",
...
for pg in sorted(check["peer_group"].dropna().unique()):
    ...
    print(subset[["Hospital Type", "Hospital Ownership", "Emergency Services",
                   "Meets criteria for birthing friendly designation"]].value_counts(dropna=False).head(3))